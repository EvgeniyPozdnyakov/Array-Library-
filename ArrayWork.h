...
{
	ArrayWork<classType> *CurDelete = ((ArrayWork<classType>*)this->Paramets->Prima)->next;
	for (unsigned int i = 1; i < this->Paramets->countVal; i++)
	{
		ArrayWork<classType> *NeaxtDel = CurDelete->next;
		if (CurDelete!=NULL)
		{
			delete CurDelete->GetValue();
			delete CurDelete;
		}
		CurDelete = NeaxtDel;
	}
